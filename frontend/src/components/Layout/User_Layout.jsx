...
const User_Layout = () => {
  ...
  return (
    <div className="flex h-screen bg-lime-50"> {/* Light lime background for the whole layout */}
      {/* Sidebar */}
      <div
      ...
      >
        <div className="p-4 flex flex-col h-full">
          <div className="flex justify-center items-center mb-6"> {/* Added margin at the bottom */}
            <h2 className="text-2xl font-semibold text-teal-700"> {/* Darker teal for logo text */}
              <a href={"/Home"} >
                <img
          ...
          <nav className="mt-2 flex-grow"> {/* Reduced top margin */}
            <NavLink
              to="/Home"
              end
              className={({ isActive }) =>
            ...
            >
              <Home className="mr-3 text-teal-500" size={20} /> {/* Teal icon */}
              Home
            </NavLink>
            <NavLink
              to="/Home/my-bookings"
            ...
            >
              <Calendar className="mr-3 text-teal-500" size={20} /> {/* Teal icon */}
              My Bookings
            </NavLink>
            <NavLink
              to="/Home/favorites"
            ...
            >
              <Star className="mr-3 text-teal-500" size={20} /> {/* Teal icon */}
              Favorites
            </NavLink>
            <NavLink
              to="/Home/settings"
            ...
            >
              <Settings className="mr-3 text-teal-500" size={20} /> {/* Teal icon */}
              Settings
            </NavLink>
          </nav>
          <div className="mt-auto flex items-center p-4 border-t border-lime-200"> {/* Light lime border */}
            <img src="https://picsum.photos/200" alt="User Avatar" className="w-10 h-10 rounded-full mr-3" />
            <div>
              <p className="text-teal-800 font-medium">{user && user.name}</p> {/* Darker teal username */}
              <button className="flex items-center text-gray-700 hover:text-teal-700 mt-1 cursor-pointer" onClick={handleLogout}> {/* Teal hover for logout */}
                <LogOut className="mr-2 text-teal-500" size={20} /> {/* Teal icon */}
                Logout
              </button>
            </div>
          </div>
      ...
      {/* Main Content */}
      <div className="flex-1 flex flex-col overflow-hidden md:ml-64 bg-lime-50"> {/* Light lime background */}
        {/* Top bar */}
        <header className="bg-white shadow-sm"> {/* Light lime background */}
        ...
        {/* Page Content */}
        <main className="flex-1 overflow-x-hidden overflow-y-auto bg-lime-50"> {/* Light lime background */}
